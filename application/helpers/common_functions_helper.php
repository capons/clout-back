...
# change the key of a multi-dimensional array into one of the unique keys specified in the list
function use_multi_column_as_key($array, $column)
{
	log_message('debug', 'common_functions_helper/use_multi_column_as_key');
	$final = array();
	if(!empty($array)) $firstItem = current($array);
	
	if(!empty($firstItem[$column])){
		foreach($array AS $row) $final[$row[$column]] = $row;
	}
