...
class _query_reader extends CI_Model
{
	...
	public function __construct()
    {
		...
		# write DB connection
        $this->dbWrite = $this->load->database(GROUPNAME_MYSQL_WRITE,TRUE);
		# read DB connection
		$this->dbRead = $this->load->database(GROUPNAME_MYSQL_READ,TRUE);
		# memcached for read queries
		if(extension_loaded('memcached')){
			log_message('debug', '_query_reader/__construct:: [1] memcached for read queries');
			$this->memcached = new Memcached();
			$this->memcached->addServer(HOSTNAME_MYSQL_READ, 11211);
		}
	...
	private function get_raw_query_string($queryCode)
	{
		...
		# Get the query from the database by the query code.  Read queries from write DB
	...
	public function load_queries_into_cache()
	{
		...
		$queries = $this->dbRead->query("SELECT * FROM queries")->result_array(); # read queries from write DB
	...
	function get_mongo_query_by_code($queryCode, $queryData = array())
	{
		...
		# test 1 - query in database
		return $this->get_list('mongodb__test_in_condition');
		
		# test 2 - query right here - for more quicker tests
		...
		#return $this->_mongo->mysql_to_mongo("SELECT store_id, name, address FROM bname WHERE name LIKE 'krankies%' AND address LIKE '211 e 3rd%' LIMIT 0,5");
